...
对外 **`/scan`** 由 `gz_laser_scan_relay` 发布，`header.frame_id` 应为 **`laser_link`**（与 URDF 一致）。桥接器在 ROS 侧先发布 **`/gz_bridge/scan_raw`**（Gazebo 作用域 frame，仅供 relay 订阅）。默认 **`align_scan_stamp_to_odom:=false`**：`gz_laser_scan_relay` 会用 **`get_clock().now()`**（与 `gazebo_topic_relay` 的 `/odom`/TF 同一仿真时钟）写 `/scan` 时间戳，并保证单调递增，避免 Cartographer 的 `Ignored subdivision…` 与桥接自带 stamp 混时间基。若用 AMCL 且必须与每条 odom 同拍，再设 **`align_scan_stamp_to_odom:=true`**（Cartographer 则可能再出现重复 stamp 告警）。

Cartographer 若仍报 `Queue waiting for data: (0, scan)`：先确认 **`ros2 topic hz /scan`** 有频率，再对照上面对齐 `gz_lidar_topic`。若报 `source_frame does not exist`，核对上一行 `frame_id` 是否为 `laser_link`。
